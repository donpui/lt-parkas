...
1. **`convert.js`** — one-time Node.js script that converts `Atviri_TP_parko_duomenys.csv` (918MB, 2.4M rows, 69 columns) into `vehicles.parquet` using DuckDB with ZSTD compression.

2. **`index.html`** — single-page app that loads DuckDB-WASM from CDN, fetches the Parquet file, and provides:
   - **5 main filters** (cascading): Markė, Komercinis pav., Gamintojo pav., Gamintojo pav. baz., Modelio metai — searchable dropdowns with type-to-filter
